...
class Contract(models.Model):
    ...
    def _get_job_count(self):
        for each in self:
            ...
            each.job_count = len(obj_pool)

    def _get_invoice_count(self):
        for each in self:
            contract_id = each.id
            obj_pool = self.env['account.move'].search([('contract_id', '=', contract_id)])
            each.invoice_count = len(obj_pool)

    def _get_journal(self):
        journal_pool = self.env['account.journal']
        company_id = (self.company_id or self.env.company).id
        journal_id = journal_pool.search([('type', '=', 'sale'), ('company_id', '=', company_id)], limit=1)
        if not journal_id:
    ...
    def button_create_invoice(self):
        ...
        for x in range(1, numbers + 1):
            ...
            invoice_line_val['company_id'] = self.company_id.id
